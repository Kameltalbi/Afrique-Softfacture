...
import type { PdfTemplateId } from '@/lib/pdf-plan-config';
import {
  ClassicTemplatePreview,
  ModernTemplatePreview,
  MinimalTemplatePreview,
} from '@/components/settings/pdf-template-previews';
...
type Props = {
  ...
  labels: {
    ...
    templates: Record<PdfTemplateId, { name: string; desc: string }>;
    logoTitle: string;
    logoSize: string;
...
export function DocumentAppearancePanel({
  ...
  const templateThumb: Record<PdfTemplateId, ReactNode> = {
    CLASSIC: <ClassicTemplatePreview className="w-full" />,
    MODERN: <ModernTemplatePreview className="w-full" />,
    MINIMAL: <MinimalTemplatePreview className="w-full" />,
  };
    ...
    <aside className="flex max-h-[48vh] w-full shrink-0 flex-col overflow-hidden border-t border-slate-200 bg-white xl:h-full xl:max-h-none xl:w-[320px] xl:border-l xl:border-t-0">
      ...
      <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto">
        <section className="border-b border-slate-100 px-5 py-4">
          <p className="mb-1 text-xs font-semibold text-slate-800">{labels.templateTitle}</p>
          <p className="mb-3 text-[11px] leading-relaxed text-slate-500">{labels.templateHint}</p>
          <div className="space-y-2.5">
            {allowedTemplates.map((id) => {
              const active = selectedTemplate === id;
              const meta = labels.templates[id];
              return (
                <button
                  key={id}
                  type="button"
                ...
                >
                  <div className="flex gap-2.5">
                    <div className="w-14 shrink-0">{templateThumb[id]}</div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {active ? (
                          <span className="rounded-full bg-brand-blue/10 px-1.5 py-0.5 text-[10px] font-semibold text-brand-blue">
                            {labels.templateActive}
                          </span>
                        ) : null}
                      </div>
                      <p className="mt-0.5 text-[11px] leading-snug text-slate-500">{meta.desc}</p>
                    </div>
                  </div>
                </button>
              );
