...
function initTemplates(company: OrgAppearanceState): TemplateMap {
  const unified = normalizePdfTemplateId(company.invoicePdfTemplate);
  return {
    invoice: unified,
    quote: unified,
    other: unified,
...
export function DocumentAppearanceSection({
  ...
  }, [limits.allowAccentColor, accents.invoice]);
  ...
  const previewTemplate = useMemo(
    () => catalogToPreviewTemplate(templates.invoice),
    [templates.invoice]
  );
  ...
  function onTemplateChange(id: PdfTemplateId) {
    if (readOnly) return;
    const next = normalizePdfTemplateId(id);
    if (!limits.allowedTemplates.includes(next)) return;
    setTemplates({ invoice: next, quote: next, other: next });
    setDirty(true);
  }
  ...
  async function onSave() {
    ...
    setSaving(true);
    try {
      const unifiedTemplate = templates.invoice;
      const unifiedColor = accents.invoice;
      const body: Record<string, unknown> = {
        pdfPrimaryColor: normalizeHexColor(unifiedColor),
        pdfFontFamily: fontFamily,
        pdfAppearance: serializePdfAppearanceStore({
          ...appearanceStore,
          mode: 'unified',
        }),
        invoicePdfTemplate: unifiedTemplate,
        quotePdfTemplate: unifiedTemplate,
        otherDocumentPdfTemplate: unifiedTemplate,
  ...
  const panelLabels = {
    ...
    templateTitle: t('appearanceTemplate'),
    templateHint: t('templateUnifiedHint'),
    templateActive: t('templateActive'),
    templates: Object.fromEntries(
      PDF_TEMPLATE_IDS.map((id) => [
        id,
        { name: t(`template_${id}`), desc: t(`templateDesc_${id}`) },
      ])
    ) as Record<PdfTemplateId, { name: string; desc: string }>,
  };
      ...
      >
        ...
        {panelOpen ? (
          <DocumentAppearancePanel
            ...
            dirty={dirty}
            saving={saving}
            selectedTemplate={templates.invoice}
