...
import { formatEur, isPlanId, isPaidPlanId, PLAN_IDS, type PlanId } from '@/lib/pricing-plans';
import {
...
export function CheckoutForm({ initialPlan, initialBilling }: Props) {
  ...
  const isFreePlan = !isPaidPlanId(plan);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.mode === 'free') {
        router.push('/checkout/success?mode=free');
        return;
      }
