...
/** Formulaires de saisie devis / facture : mode focus (sidebar masquée). */
...
export function AppShell({
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <aside
        className={cn(
          'fixed inset-y-0 start-0 z-40 flex flex-col border-e border-slate-200 bg-white transition-[width] duration-300 ease-out',
          documentFocusMode
      ...
      <div
        className={cn(
          ...
          documentFocusMode ? 'ms-0' : sidebarOpen ? 'ms-60' : 'ms-16'
        )}
      >
        <header
          className={cn(
            'sticky top-0 z-20 grid h-14 grid-cols-[1fr_auto_1fr] items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-6',
        ...
        >
          ...
          <time
            ...
            className="flex items-center justify-center gap-6 tabular-nums text-sm font-semibold"
          >
        ...
        <main
          className={cn(
            'min-w-0 flex-1',
            documentFocusMode ? 'bg-slate-100/80 p-3 md:p-6' : 'p-4 md:p-6'
          )}
        >
          {children}
        </main>
