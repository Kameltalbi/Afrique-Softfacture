...
import { NewClientForm, type CreatedClient } from '@/components/clients/new-client-drawer';
import {
...
export type InvoiceEditorPanel = 'settings' | 'notes' | 'tools' | 'help' | 'client' | null;
...
type InvoiceEditorShellProps = {
  ...
  /** Ouverture « Créer un client » depuis la feuille document → panneau droit. */
  ...
  return (
    <div className={cn('min-w-0', DOCUMENT_EDITOR_RAIL_BLEED_CLASS)}>
      ...
      >
        ...
        >
          {activePanel === 'client' && onClientCreated ? (
            <NewClientForm
              variant="dock"
