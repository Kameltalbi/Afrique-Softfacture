...
import { InvoiceHeroVisual } from '@/components/marketing/feature-hero-visuals';
import { getTranslations } from 'next-intl/server';

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('featurePages.factures');
  return {
    title: t('metaTitle'),
...
export default async function FacturesMarketingPage() {
  const t = await getTranslations('featurePages.factures');

  return (
    <MarketingShell activeNav="invoices">
      <FeaturePageHero
        tone="invoices"
        label={t('label')}
        title={
