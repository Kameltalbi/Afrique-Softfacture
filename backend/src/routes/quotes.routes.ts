...
import { planAllowsCustomLogo, planShowsPdfWatermark } from '../lib/planFeatures.js';
...
router.get('/:id/pdf', async (req, res) => {
  ...
  streamQuotePdf(res, {
    ...
    notes: q.notes,
    currency: q.currency,
    template: q.organization.invoicePdfTemplate,
    accentColor: resolvePdfAccent(q.organization, 'invoice'),
    footerText: q.organization.documentFooterText,
    defaultFooterLine: `${APP_BRAND} — Devis généré automatiquement.`,
    showWatermark: planShowsPdfWatermark(q.organization.subscriptionPlan),
    org: {
      name: q.organization.name,
      logoUrl: planAllowsCustomLogo(q.organization.subscriptionPlan)
        ? q.organization.logoUrl
        : null,
