...
import { getMarketByCountry, MARKET_COUNTRY_CODES } from '../lib/markets.js';
...
const registerSchema = z.object({
  ...
  country: z.enum(MARKET_COUNTRY_CODES),
});
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const { email, password, firstName, lastName, organizationName, phone, country } = parsed.data;
    ...
    const result = await prisma.$transaction(async (tx) => {
      const org = await tx.organization.create({
        data: {
          ...
          defaultVatRate: market.defaultVatRate,
        },
      });
