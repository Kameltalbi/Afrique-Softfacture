...
import { assertCanAddClient, ClientLimitError } from '../lib/planLimits.js';
...
router.post('/import', async (req, res) => {
  ...
  for (let i = 0; i < parsed.data.rows.length; i += 1) {
    ...
    try {
      ...
      try {
        await assertCanAddClient(organizationId);
      } catch (err) {
...
router.post('/', async (req, res) => {
  ...
  try {
    await assertCanAddClient(orgId(req));
  } catch (err) {
    if (err instanceof ClientLimitError) {
      return res.status(403).json({ error: err.message, code: err.code, max: err.max });
    }
    throw err;
  }
