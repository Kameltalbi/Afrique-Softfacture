...
function drawTableHeaderClassic(
  ...
  // Bande pleine largeur page (évite l'effet « coupé » aux marges)
  doc.save();
...
function drawTableHeaderMinimal(
  ...
  // Pleine largeur de la page A4 — bandeau pro, plus d'effet « coupé »
  doc.save();
...
function renderMinimal(doc: InstanceType<typeof PDFDocument>, inv: InvoicePdfInput): void {
  ...
  doc.text(`${inv.vatTotal} ${inv.currency}`, totalsX + 80, yTotals + 26, {
    width: 110,
    align: 'right',
    lineBreak: false,
  });
...
function renderInvoiceToDocument(doc: InstanceType<typeof PDFDocument>, inv: InvoicePdfInput) {
  const template =
    inv.template === 'BLUE_PRO' ? 'MODERN' : inv.template === 'MONO' ? 'MINIMAL' : inv.template;
  switch (template) {
    case 'MODERN':
